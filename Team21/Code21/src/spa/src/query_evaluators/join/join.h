...
class HashJoin {
 public:
  ...
  Table operator()(const Table &hash_table, const Table &probe_table) {
    ...
    for (std::size_t probe_row_idx = 0; probe_row_idx < probe_table.ResultSize(); ++probe_row_idx) {
      ...
      for (auto hashed_row_idx : matched_key->second) {
        ...
        for (const auto &key : combined_keys) {
          if (hash_table.HasKey(key)) row.push_back(hash_table.GetCell(hashed_row_idx, key));
          else row.push_back(probe_table.GetCell(probe_row_idx, key));
        }
