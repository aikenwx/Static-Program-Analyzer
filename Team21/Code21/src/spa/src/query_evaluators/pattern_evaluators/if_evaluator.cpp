...
namespace qps {
  std::vector<Product> IfEvaluator::CallPkb(QueryFacade& pkb) {
    ...
    if (std::holds_alternative<QuotedIdentifier>(ref1)) {
      std::set<IfStatement*>* set = pkb.getIfStatementsUsingVariableInCondition(std::get<QuotedIdentifier>(ref1).getQuotedId());
      ...
      auto all_if_variable_pairs =
        pkb.getUsesRelationshipsByLeftAndRightEntityTypes(IfStatement::getEntityTypeStatic(), Variable::getEntityTypeStatic());
