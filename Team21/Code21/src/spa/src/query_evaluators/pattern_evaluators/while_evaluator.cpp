...
namespace qps {
  std::vector<Product> WhileEvaluator::CallPkb(QueryFacade& pkb) {
    ...
    if (std::holds_alternative<QuotedIdentifier>(ref1)) {
      std::set<WhileStatement*>* set = pkb.getWhileStatementsUsingVariableInCondition(std::get<QuotedIdentifier>(ref1).getQuotedId());
      ...
      auto all_while_variable_pairs =
        pkb.getUsesRelationshipsByLeftAndRightEntityTypes(WhileStatement::getEntityTypeStatic(), Variable::getEntityTypeStatic());
