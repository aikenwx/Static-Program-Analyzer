...
auto WithDoubleAttrEvaluator::CallPkb(QueryFacade &pkb) -> std::vector<std::vector<Entity *>> {
  ...
  const auto &decl = getDeclarations();

  const auto &ref1Attr = std::get<AttrRef>(ref1.ref);
  const auto &ref2Attr = std::get<AttrRef>(ref2.ref);
  ...
  const auto &ent1 = DesignEntityToEntityType(decl1.value().getDesignEntity());
  const auto &ent2 = DesignEntityToEntityType(decl2.value().getDesignEntity());
