...
TEST_CASE("Semantic validation for queries") {
  ...
  SECTION("The synonym for select clause is not previously declared") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Semantic error. There is missing declaration in Select clause for synonym S"));
  }
  ...
  SECTION("The synonym for assignPattern clause is not previously declared") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring(
                            "Semantic error. There is missing declaration in Pattern clause for argument 1"));
  }
  ...
  SECTION("check pattern clause has proper synonym") {
    ...
    qps::SemanticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Semantic error. Invalid syntax for pattern with synonym: p"));
  }
  ...
  SECTION("check that attrName is acceptable with given synonym") {
    ...
    qps::SemanticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Semantic error. Attribute name is invalid with synonym s"));
  }
  ...
  SECTION("check synonym BOOLEAN have declaration") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring(
                            "Semantic error. There is missing declaration in Select clause for synonym BOOLEAN"));
  }
  ...
  SECTION("check synonym BOOLEAN have declaration") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring(
                            "Semantic error. There is missing declaration in Select clause for synonym (in AttrRef) s"));
  }
  ...
  SECTION("check synonym BOOLEAN in attrRef have declaration") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring(
                            "Semantic error. There is missing declaration in Select clause for synonym (in AttrRef) BOOLEAN"));
  }
  ...
  SECTION("check that withRef in WithClause have declaration") {
    ...
    qps::SemanticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Semantic error. There is missing declaration for synonym (in WithRef) c"));
  }
  ...
  SECTION("check that attrName is acceptable with synonym in WithClause") {
    ...
    qps::SemanticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Semantic error. Attribute name is invalid with synonym c"));
  }
  ...
  SECTION("check that correct comparison of withRef in WithClause") {
    ...
    qps::SemanticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Semantic error. Comparison of different type of attribute in with clause"));
  }
  ...
  SECTION("check incorrect pattern with not if/assign/while synonym") {
    ...
    qps::SemanticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Semantic error. Invalid syntax for pattern with synonym: w"));
  }
  ...
  SECTION("check pattern if/assign/while synonym have declaration") {
    ...
    REQUIRE_THROWS_WITH(QueryHelper::buildQuery(dupeInput),
                        ContainsSubstring("Semantic error. There is missing declaration in pattern clause for synonym a"));
  }
...
TEST_CASE("Syntactic validation for queries") {
  ...
  SECTION("check parent cannot have quoted identifier as argument 2") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Syntactic error. The argument is not of correct ref type for Parent"));
  }
  ...
  SECTION("check follow* cannot have quoted identifier as argument 2") {
    ...
    qps::SyntacticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Syntactic error. The argument is not of correct ref type for Follows*"));
  }
  ...
  SECTION("check modifies cannot have statement number as argument 2") {
    ...
    qps::SyntacticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Syntactic error. The argument is not of correct ref type for Modifies(S)"));
  }
  ...
  SECTION("check uses cannot have statement number as argument 2") {
    ...
    qps::SyntacticValidator validator(dupeQuery);
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring("Syntactic error. The argument is not of correct ref type for Uses(P)"));
  }
  ...
  SECTION("check AssignPattern have correct ref types") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring(
                            "Syntactic error. The first argument is not of correct ref type for assign clause"));
  }
  ...
  SECTION("check that attrRef (attrName) is incorrect") {
    std::string dupeInput("stmt s; procedure p; variable P; assign S; Select <s.stmt, p.procName, P>");
    REQUIRE_THROWS_WITH(QueryHelper::buildQuery(dupeInput),
                        ContainsSubstring("Syntactic error. Invalid Attribute Name stmt"));
  }
  ...
  SECTION("check incorrect arg2 with while pattern") {
    ...
    REQUIRE_THROWS_WITH(validator.validateQuery(),
                        ContainsSubstring(
                            "Syntactic error. The second argument of if/while pattern cannot be expression"));
  }
  ...
  SECTION("check incorrect arg3 with if pattern") {
    ...
    REQUIRE_THROWS_WITH(QueryHelper::buildQuery(dupeInput),
                        ContainsSubstring("Syntactic error. Invalid Query Syntax. Expect (_) got (\")"));
  }
}
